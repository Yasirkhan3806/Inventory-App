...
export const setPerson = async (data) => {
    try{
        console.log("Data in setPerson",data);
        const {name} = data;
        const personExists = await personModel.findOne({name});
...
export const deletePerson = async (id) => {
    try{
        const person = await personModel.findByIdAndUpdate(id,{active:false},{new:true});
